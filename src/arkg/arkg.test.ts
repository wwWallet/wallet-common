		...
		describe(`instance ${instanceName}`, async () => {
			...
			describe("deriveSeed", () => {
				it("derives the same results on repeat calls.", async () => {
					...
					assert.deepEqual(await exportPubSeed(pub_seed_1), await exportPubSeed(pub_seed_2));
				...
				it("derives different results on calls with different ikm.", async () => {
					...
					assert.notDeepEqual(await exportPubSeed(pub_seed_1), await exportPubSeed(pub_seed_2));
